    ...
    <div className="app_accountChange app_profile app_forms w-12/12 lg:w-10/12 xl:w-9/12 sm:mx-auto mt-20 pt-[20px] md:pt-[51px] pb-[25px] md:pb-[55px] px-10 md:pl-[44px] rounded-[30px] md:pr-[63px] bg-white">
      <div className="text-center">
        <div className="text-[40px] font-[600] relative flex">
          ...
          /> */}
          <img
            src={isSelected ? `${URL.createObjectURL(selectedFile)}` : user?.profile?.profile_image ? `${BASE_URL}${user.profile.profile_image}` : person}
            alt="Person"
            className="rounded-full w-[125px] border-[0.5px]"
          />
          ...
          <input id="profile_image" type="file" name="image" className="hidden" onChange={changeFileHandler} />
          </span>
        ...
        <form className="app_forms-form mt-[53px] text-[15px] md:text-[25px]" onSubmit={updateProfile}>
          ...
          />
          <button className="main_btn mt-0 bg-pink-600 text-white" type="submit">Save Changes</button>
        </form>
      </div>
    </div>
  );
