...
import { BASE_URL, GET_A_BLOG_URL, GET_ALL_ARTICLE_URL, GET_MY_PROFILE_URL } from "../commons/constant";
  ...
  return (
    <div className="app_blogarticle mt-20">
      ...
      <div className="flex">
        <div className="app_blog-context w-full lg:w-4/6 pr-6">
          <div className="app_blogarticle-author flex items-center flex-wrap">
            <div className="app_blogarticle-author_img w-6/6 md:w-3/6 flex items-center">
              <img src={blog?.author?.profile?.profile_image ? `${BASE_URL}${blog?.author?.profile?.profile_image}` : person} alt="Author" className="w-16 rounded-full" />
              <div className="app_blogarticle-author_info ml-4">
                <h2 className="text-[17px]">{blog.author ? `${blog?.author?.first_name}${blog?.author?.last_name}` : "No Author"}</h2>
                <h4 className="text-[14px]">{blog?.timestamp && moment(blog?.timestamp).format("MMMM D, YYYY")} | { blog?.total_minute_read < 60 ? `${blog?.total_minute_read} min`  : `${blog?.total_minute_read / 60} hour`} read.</h4>
              </div>
              <div
          ...
          <article className="app_blogarticle-article">
            ...
            <section className="app_blogarticle-article_image">
              <img src={`${BASE_URL}${blog.image}`} alt="Article" className="w-[80%]" />
            </section>
        ...
        <div
          className={`w-2/6 border-l-2 pl-8 h-[${document.querySelector(".app_blogarticle") &&
