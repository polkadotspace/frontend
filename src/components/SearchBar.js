...
const SearchBar = ({ setSearchText }) => {
  ...
  const renderDeleteSearch = () => {
    if (window.location.href.includes("search")) {
      return (
        <>
          <img
            className={`${searchValue && searchValue.length > 0 ? "block" : "hidden"
          ...
          />
          <span
            className={`${searchValue && searchValue.length > 0 ? "inline-block" : "hidden"
  ...
  return (
    <div
      className={`app_search-input ${window.location.href.includes("search") ? "w-5/6" : "w-full"
        } relative overflow-hidden`}
    >
      <span
        className={`absolute ${window.location.href.includes("search") ? "left-[2%]" : "left-[11%]"
      ...
      {renderDeleteSearch()}
      <span
        className={`absolute ${window.location.href.includes("search")
          ? "right-[20%]"
          : "right-[11%]"
