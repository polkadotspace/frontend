      ...
      <div className="app_blog-box border-b-2 flex md:pb-[25px] pl-[16px] pb-[15px] justify-between mt-[25px]">
        <div className="app_blog-box_info w-4/6">
          <div className="app_blog-box_author flex items-center">
            <img
              src={blog?.author?.profile?.profile_image ? `${BASE_URL}${blog?.author?.profile?.profile_image}` : person}
              alt="Person"
            ...
            <span className="text-[8px] md:text-[15px] text-gray-400">
              {moment(blog?.timestamp).format("dddd, MMMM Do YYYY, h:mm:ss a")}
            </span>
          </div>
          <div className="app_blog-box_title text-[14px] md:text-[22px] font-[700] my-3">
            <a href={`/pages/blogarticle/${blog.id}`}>
              {blog?.title}
            </a>
          </div>
          ...
          <div className="app_blog-box_tags mt-[25px] lg:mt-[58px] flex">
            {
              blog?.keywords?.split(",").map((keyword, index) => (
                <span key={index} className="py-[4px] md:py-[9px] px-[8px] md:px-[30px] text-[10px] md:text-[15px] cursor-pointer bg-[#D9D9D9] rounded-[34px]">
                  {keyword}
                </span>
