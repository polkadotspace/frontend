...
const Comments = ({ openComments, setOpenComments }) => {
  ...
  return (
    <div
      className={`app_comments fixed h-screen px-8 py-10 w-[400px] right-0 transition-all duration-200 ${
        openComments ? "translate-x-full" : "translate-x-0"
      } top-0 overflow-y-scroll border-l-2 bg-white`}
    >
      <header className="flex justify-between text-[19px] ">
        <h2>Comments (20)</h2>
        <h3
      ...
      />
      {/* Separator */}
      <div className="border-b-2 w-full h-2 absolute top-[50%] left-0"></div>
      {/* User Comment */}
      <div className="mt-12">
